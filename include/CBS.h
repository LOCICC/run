...
class TreeSaltBuilder : public CBSInterface
{
 public:
  ...
  void rungpu(const salt::Net& net, salt::Tree& input_tree, double eps, int refine_level = 3);
 protected:
...
class TempBuilder : public CBSInterface
{
 ...
 protected:
  ...
  static double calcElmoreDelay(const double& cap, const double& len) //TimingPropagator.hh
  {
    double _unit_res = 7.83333e-05;
    double _unit_cap = 8.577e-5;
  ...
  double delay(const T1& from, const T2& to) const
  {
    ...
    auto to_loc = to.loc;
    auto len = utils::Dist(from_loc, to_loc) / 1000.;
    auto cap_load = _cap_loads[to.id];
    return calcElmoreDelay(cap_load, len);
  }
  template <DelayType T1, DelayType T2>
