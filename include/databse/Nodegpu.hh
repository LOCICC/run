...
class Node
{
 public:
  ...
  {
    func(this);
    std::for_each(_children.begin(), _children.end(), [&](auto child) { child->preOrder(func); });
  }
  ...
  {
    std::for_each(_children.begin(), _children.end(), [&](auto child) { child->postOrder(func); });
    func(this);
  }
 ...
 const std::string& get_cell_master() const { return _cell_master; }
  ...
  virtual bool isLoad() const { return _ptype==PinType::kLoad; }
  ...
  void set_ptype(const PinType& type) { _ptype = type; }
  int _id = 0;
  int instID = -1;
  std::string _name = "";
