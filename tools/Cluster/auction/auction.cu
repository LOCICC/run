                                        ...
                                        const int max_iterations)
{
    ...
    while(auction_eps >= auction_min_eps && num_assigned < num_nodes&&num_iteration < max_iterations)
    {
        ...
        while(num_assigned < num_nodes && num_iteration < max_iterations)
        {
            ...
            for(int i = node_id; i < num_nodes; i += blockDim.x){
                s_prices[i] = prices[i];
                // if(i==200)printf("%d %d %f\n",i,blockDim.x,s_prices[i]);
            }
            __syncthreads();
            ...
            if(person2item[i] == -1){
                ...
                // if(node_id==0)printf("person2item[%d]:%d\n",i,top1_col);
            }
            }
            ...
            if(sbids[j] != 0) {
                ...
                // if(node_id==0)printf("item:%d person:%d\n",j,item2person[j]);
                ...
                const int max_iterations)
{
    ...
    int* person2item,*item2person,*sbids;
    double *prices,*bids;
    ...
    //init
    cudaMemsetAsync(prices, 0, num_graphs * num_nodes * sizeof(double));

    //launch solver
    linear_assignment_auction_kernel<double><<<1, 1000>>>
                                    (
                                        num_nodes,
                                        cost_matrics,
                                        person2item,
